...
static void drawPattern(void *info, CGContextRef context) {
  UIImage *patternImage = [UIImage
      imageWithContentsOfFile:[[NSBundle mainBundle]
                                  pathForResource:[patternName lowercaseString]
                                           ofType:@"png"]];
  CGAffineTransform flip = CGAffineTransformMakeScale(1, -1);
  CGAffineTransform shift =
      CGAffineTransformTranslate(flip, 0, patternImage.size.height * -1);
  CGContextConcatCTM(context, shift);

  CGContextDrawImage(context, CGRectMake(0, 0, patternImage.size.width,
                                         patternImage.size.height),
                     patternImage.CGImage);
}
...
static void drawTextWithImagePattern(CGContextRef context, NSString *imageName,
  ...
  CTFontRef myCFFont =
      CTFontCreateWithName(fontName, bounds.size.height * .15, NULL);
  CFAutorelease(myCFFont);

  NSDictionary *patternDictionary =
      [NSDictionary dictionaryWithObjectsAndKeys:(__bridge id)myCFFont,
                                                 (id)kCTFontAttributeName, nil];

  CFAttributedStringRef attrString = CFAttributedStringCreate(
      kCFAllocatorDefault, (__bridge CFStringRef)imageName,
      (__bridge CFDictionaryRef)patternDictionary);
  ...
  CGPatternRef plaidPattern = CGPatternCreate(
      NULL, bounds, CGAffineTransformIdentity, imageSize.width,
      imageSize.height, kCGPatternTilingConstantSpacing, false, &callbacks);
...
- (void)drawDemoIntoContext:(CGContextRef)context withFrame:(CGRect)bounds {
  ...
  drawTextWithImagePattern(context, @"PLAID", bounds, CGSizeMake(97, 95), 0.7f,
                           (__bridge CFStringRef) @"Segoe UI Bold");
  drawTextWithImagePattern(context, @"DENIM", bounds, CGSizeMake(500, 300),
                           0.5f, (__bridge CFStringRef) @"Algerian");
  CGContextTranslateCTM(context, -.27 * bounds.size.width, 0);
  drawTextWithImagePattern(context, @"CHECKERED", bounds, CGSizeMake(16, 16),
                           0.28f, (__bridge CFStringRef) @"Arial Bold");
  ...
  CTFontRef mainFont =
      CTFontCreateWithName(@"Times New Roman", bounds.size.height * .04, NULL);
  CTFontRef fontRangeFont =
      CTFontCreateWithName(@"Blackadder ITC", bounds.size.height * .04, NULL);
  CTFontRef masterRangeFont =
      CTFontCreateWithName(@"Magneto Bold", bounds.size.height * .04, NULL);
  CFAutorelease(mainFont);
  CFAutorelease(fontRangeFont);
  CFAutorelease(masterRangeFont);

  NSDictionary *attributesDict = [NSDictionary
      dictionaryWithObjectsAndKeys:(__bridge id)mainFont,
                                   (id)kCTFontAttributeName,
                                   [UIColor blackColor].CGColor,
                                   (id)kCTForegroundColorAttributeName,
                                   (__bridge id)paragraphRef,
                                   kCTParagraphStyleAttributeName, nil];

  CFAttributedStringRef attrString = CFAttributedStringCreate(
      kCFAllocatorDefault, (__bridge CFStringRef)textBox,
      (__bridge CFDictionaryRef)attributesDict);
  CFMutableAttributedStringRef mutableAttrString =
      CFAttributedStringCreateMutableCopy(kCFAllocatorDefault, [textBox length],
                                          attrString);
  ...
  CFAttributedStringSetAttribute(
      mutableAttrString, CFRangeMake(attrRange.location, attrRange.length),
      kCTForegroundColorAttributeName, [UIColor blueColor].CGColor);
  CFAttributedStringSetAttribute(
      mutableAttrString, CFRangeMake(mineRange.location, mineRange.length),
      kCTForegroundColorAttributeName, [UIColor redColor].CGColor);
  CFAttributedStringSetAttribute(
      mutableAttrString, CFRangeMake(manyRange.location, manyRange.length),
      kCTForegroundColorAttributeName, [UIColor greenColor].CGColor);
  CFAttributedStringSetAttribute(
      mutableAttrString, CFRangeMake(fontRange.location, fontRange.length),
      kCTFontAttributeName, fontRangeFont);
  CFAttributedStringSetAttribute(
      mutableAttrString, CFRangeMake(masterRange.location, masterRange.length),
      kCTFontAttributeName, masterRangeFont);
  CFAttributedStringEndEditing(mutableAttrString);

  CTFramesetterRef framesetter =
      CTFramesetterCreateWithAttributedString(mutableAttrString);
  CFAutorelease(framesetter);

  CGMutablePathRef textPath = CGPathCreateMutable();
  CGContextTranslateCTM(context, 0.1 * bounds.size.width,
                        0.1 * bounds.size.height);
  CGPathAddRect(textPath, NULL, CGRectMake(0, 0, bounds.size.width * .8,
